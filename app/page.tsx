...
export default function Home(){
 ...
 return <main className="shell"><aside className="sidebar"><div className="brand"><span className="brand-mark" style={{background: 'transparent', width: '24px', height: '24px', display: 'inline-block', marginRight: '8px', color: '#00baf2'}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg></span> SAHYOGI</div><div className="workspace"><span className="dot"/>Paytm Innovation Lab</div><nav><button className={tab==="home"?"active":""} onClick={()=>navigate("home")}>⌂ Home</button><button className={tab==="sahyogis"?"active":""} onClick={()=>navigate("sahyogis")}>◉ Sahyogis</button><button className={tab==="roi"?"active":""} onClick={()=>navigate("roi")}>↗ ROI dashboard</button></nav><div className="sidebar-bottom"><div className="you"><span className="avatar small">RM</span><div><b>Raj Motwani</b><small>Product team</small></div></div></div></aside><section className="content"><header><div><p className="eyebrow">PAYTM INNOVATION LAB</p><h1>{tab==="sahyogis"?"Your Sahyogi organisation":tab==="roi"?"Business impact":"Good morning, Raj."}</h1></div><button className="new-button">+ Hire Sahyogi</button></header>
 {tab==="home"&&<><section className="hero" style={{backgroundImage: "url('/hero-bg.png')", backgroundSize: "cover", backgroundPosition: "center", color: "white"}}><div className="hero-copy" style={{background: "rgba(0,41,112,0.8)", padding: "20px", borderRadius: "12px", border: "1px solid rgba(0, 186, 242, 0.3)"}}><span className="pill" style={{background: "#00baf2", color: "white", border: "none"}}>✦ PAYTM AI WORKFORCE</span><h2 style={{color: "white"}}>Autonomous Fintech <em>Scale.</em></h2><p style={{color: "#e6f7ff"}}>Empowering Paytm’s future with highly-contextual, autonomous Sahyogis working 24/7.</p><div className="ask"><span>✦</span><input value={prompt} onChange={e=>setPrompt(e.target.value)} onKeyDown={e=>e.key==="Enter"&&askFastAPI()} placeholder="Ask your Sahyogi anything…" disabled={isAsking}/><button onClick={askFastAPI} disabled={isAsking}>↑</button></div>
  ...
  {answer && !isAsking && !askError && <div className="reply"><p style={{color:'#00baf2', fontSize:'12px', lineHeight:1.6}}><b>Sahyogi says:</b> {answer}</p>{sources.length>0 && <p style={{marginTop:'8px', color:'#cceeff'}}><b>Sources:</b> {sources.map(s=><a href={s.url} key={s.url} style={{marginRight:'8px', color:'#00baf2'}}>{s.name}</a>)}</p>}</div>}
  </div></section>
  <section className="metrics"><div><span style={{background: "#00baf2", color: "white", borderRadius: "4px", padding: "4px 8px"}}>✦</span><p>Transactions Monitored</p><b>14.2M <small>today</small></b></div><div><span style={{background: "#002970", color: "white", borderRadius: "4px", padding: "4px 8px"}}>○</span><p>Queries Resolved</p><b>84.3k <small>this week</small></b></div><div><span style={{background: "#e6f7ff", color: "#002970", borderRadius: "4px", padding: "4px 8px"}}>◇</span><p>Hours Saved</p><b>12,450 <small>across teams</small></b></div></section>
  <section className="grid"><div className="panel"><p className="eyebrow">LIVE WORKFORCE ACTIVITY</p><h3>Sales-AI</h3><p>Prospecting 500 Enterprise Leads for POS Growth. Outbound campaigns actively sending.</p><button className="primary" style={{background: "#002970", color: "white"}}>View Live Campaigns →</button></div><div className="panel"><p className="eyebrow">SUGGESTED ESCALATIONS</p><h3>Risk-AI: Anomaly Detected</h3><p>Detected unusual transaction volume in region IN-WEST. Human review required before freezing accounts.</p><button className="primary" style={{background: "#00baf2", color: "white"}}>Review Anomalies →</button></div></section></>}
